...
public class BeforeInvoke extends InjectionPoint {
   ...
   public boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes) {
      ...
      boolean found = this.find(desc, insns, nodes, this.target, BeforeInvoke.SearchType.STRICT);
      if (!found && hasDescriptor && this.allowPermissive) {
         ...
         found = this.find(desc, insns, nodes, this.target, BeforeInvoke.SearchType.PERMISSIVE);
      }
   ...
   protected boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes, ITargetSelector selector, SearchType searchType) {
      if (selector == null) {
         ...
         ITargetSelector target = (searchType == BeforeInvoke.SearchType.PERMISSIVE ? selector.configure(ITargetSelector.Configure.PERMISSIVE) : selector).configure(ITargetSelector.Configure.SELECT_INSTRUCTION);
         int ordinal = 0;
         int found = 0;
         int matchCount = 0;
   ...
   public static enum SearchType {
      ...
      private static SearchType[] $values() {
         return new SearchType[]{STRICT, PERMISSIVE};
      }
   }
}
