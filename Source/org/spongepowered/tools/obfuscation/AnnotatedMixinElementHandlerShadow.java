...
class AnnotatedMixinElementHandlerShadow extends AnnotatedMixinElementHandler {
      ...
      if (obfData.isEmpty()) {
         ...
         for(ObfuscationType type : obfData) {
            try {
               ...
               elem.printMessage(this.ap, IMessagerEx.MessageType.SHADOW_MAPPING_CONFLICT, "Mapping conflict for @Shadow " + elem + ": " + ex.getNew().getSimpleName() + " for target " + target + " conflicts with existing mapping " + ex.getOld().getSimpleName());
            }
         }
   ...
   class AnnotatedElementShadowMethod extends AnnotatedElementShadow<ExecutableElement, MappingMethod> {
      ...
      public boolean shouldRemap() {
         return super.shouldRemap() && ((ExecutableElement)this.element).getKind() != ElementKind.CONSTRUCTOR;
      }
